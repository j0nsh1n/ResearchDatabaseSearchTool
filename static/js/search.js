...
async function loadClusterFilter() {
    try {
        ...
        if (data.clusters && data.clusters.length > 0) {
            ...
            data.clusters.forEach(c => {
                ...
                opt.textContent = `${c.cluster_label} (${c.article_count})`;
                select.appendChild(opt);
            });
            document.getElementById('cluster-filter-group').style.display = 'block';
        }
    } catch (e) {
        // No clusters yet, keep filter hidden
    }
}
...
function renderResults(results) {
    ...
    results.forEach((article, idx) => {
        ...
        if (article.cluster_label) {
            clusterHtml = `<span class="cluster-badge">${article.cluster_label}</span>`;
        }
...
function doExport(format) {
    ...
    if (lastSearchParams.cluster_filter) {
        params.set('cluster_filter', lastSearchParams.cluster_filter.join(','));
    }
