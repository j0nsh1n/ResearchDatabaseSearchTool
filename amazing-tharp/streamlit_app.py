    ...
    if st.session_state.search_results:
        ...
        for i, article in enumerate(st.session_state.search_results, 1):
            with st.expander(
                ...
                with col3:
                    ...
                    if article.get('source') == 'pubmed' and article_id:
                        st.markdown(f"**PMID:** [{article_id}](https://pubmed.ncbi.nlm.nih.gov/{article_id}/)")
                    else:
                        st.markdown(f"**ID:** {article_id} ({article.get('source', '')})")
        ...
        with col2:
            ...
            for i, article in enumerate(st.session_state.search_results, 1):
                ...
                text_export += f"   {article['year']} | {article['journal']} | ID: {article.get('article_id', '')} ({article.get('source', '')})\n\n"
...
# Page: Statistics
    ...
    if st.button("Detect Duplicates"):
        ...
        if duplicates:
            ...
            # Show top duplicates
            for (id1, src1), (id2, src2), sim in duplicates[:10]:
                article1 = st.session_state.pipeline.db.get_article_by_id(id1, src1)
                article2 = st.session_state.pipeline.db.get_article_by_id(id2, src2)
                
                with st.expander(f"[{sim:.3f}] Potential Duplicate"):
                    st.markdown(f"**Article 1:** {article1['title'] if article1 else 'Unknown'}")
                    st.markdown(f"*ID: {id1} ({src1})*")
                    st.markdown(f"**Article 2:** {article2['title'] if article2 else 'Unknown'}")
                    st.markdown(f"*ID: {id2} ({src2})*")
        else:
